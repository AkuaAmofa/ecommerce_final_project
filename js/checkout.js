...
function loadCheckoutSummary() {
    ...
    console.log('Fetching cart data from cart_items_action.php...');

    fetch('../actions/cart_items_action.php', {
        method: 'POST'
    })
    .then(response => {
        ...
        try {
            ...
            if (data.status === 'success') {
                if (data.items && data.items.length > 0) {
                    ...
                    const total = data.totals ? data.totals.subtotal : 0;
                    displayCheckoutItems(data.items, total);
                } else {
...
function displayCheckoutItems(items, total) {
    ...
    items.forEach((item) => {
        ...
        itemDiv.innerHTML = `
            <div class="item-details">
                ...
                <div class="item-info">Qty: ${item.qty} × GHS ${parseFloat(item.product_price).toFixed(2)}</div>
            </div>
    ...
    // Store total and items for payment modal
