...
<head>
    ...
    <title>Processing Payment - EventLink</title>
    ...
    <style>
        ...
        .container {
            ...
            padding: 60px 40px;
            border-radius: 20px;
            box-shadow: 0 20px 60px rgba(43, 58, 103, 0.15);
            text-align: center;
        }
        ...
        .spinner {
            ...
            border-top: 5px solid var(--el-gold);
            border-radius: 50%;
        ...
        async function verifyPayment() {
            ...
            try {
                ...
                if (data.status === 'success' && data.verified) {
                    ...
                    setTimeout(() => {
                        const url = `order_confirmation.php?` +
                            `invoice=${encodeURIComponent(data.invoice_no)}` +
                            `&reference=${encodeURIComponent(reference)}` +
                            `&amount=${encodeURIComponent(data.total_amount)}` +
                            `&date=${encodeURIComponent(data.order_date)}` +
                            `&items=${encodeURIComponent(data.item_count)}`;
